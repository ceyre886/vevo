  ...
  transformRequest: [function transformRequest(data, headers) {
    ...
    if (isObjectPayload) {
      ...
      if ((isFileList = utils$1.isFileList(data)) || contentType.indexOf('multipart/form-data') > -1) {
        const _FormData = this[REDACTED_FILENAME] && this[REDACTED_FILENAME].FormData;
    ...
    if (protocol === 'data:') {
      ...
      try {
        convertedData = fromDataURI(config.url, responseType === 'blob', {
          Blob: config[REDACTED_FILENAME] && config[REDACTED_FILENAME].Blob
        });
      } catch (err) {
...
const getFetch = (config) => {
  let env = config ? config[REDACTED_FILENAME] : {};
