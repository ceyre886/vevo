...
* Optionally specify `DOTENV_CONFIG_QUIET=true` in your environment or `[REDACTED_FILENAME]` file to quiet the runtime log ([#889](https://github.com/motdotla/dotenv/pull/889))
...
const TIPS = [
  ...
  '🔐 prevent committing [REDACTED_FILENAME] to code: https://dotenvx.com/precommit',
  '🔐 prevent building [REDACTED_FILENAME] in docker: https://dotenvx.com/prebuild',
  '🛠️  run anywhere with `dotenvx run -- yourcommand`',
  '⚙️  specify custom [REDACTED_FILENAME] file path with { path: \'/custom/path/[REDACTED_FILENAME]\' }',
  '⚙️  enable debug logging with { debug: true }',
  '⚙️  override existing env vars with { override: true }',
  '⚙️  suppress all logs with { quiet: true }',
  '⚙️  write to custom object with { processEnv: myObject }',
  '⚙️  load multiple [REDACTED_FILENAME] files with { path: [\'[REDACTED_FILENAME].local\', \'[REDACTED_FILENAME]\'] }'
]
```
...
* Patched injected log to count only populated/set keys to process[REDACTED_FILENAME] ([#879](https://github.com/motdotla/dotenv/pull/879))
...
- And check out the new [dotenvx](https://github.com/dotenvx/dotenvx). As coding workflows evolve and agents increasingly handle secrets, encrypted [REDACTED_FILENAME] files offer a much safer way to deploy both agents and code together with secure secrets. Simply switch `require('dotenv').config()` for `require('@dotenvx/dotenvx').config()`.

## [16.6.0](https://github.com/motdotla/dotenv/compare/v16.5.0...v16.6.0) (2025-06-26)

### Added

- Default log helpful message `[dotenv@16.6.0] injecting env (1) from [REDACTED_FILENAME]` ([#870](https://github.com/motdotla/dotenv/pull/870))
...
- 🐞 Fix recent regression when using `path` option. return to historical behavior: do not attempt to auto find `[REDACTED_FILENAME]` if `path` set. (regression was introduced in `16.4.3`) [#814](https://github.com/motdotla/dotenv/pull/814)
...
- Add `error.code` to error messages around `[REDACTED_FILENAME].vault` decryption handling [#795](https://github.com/motdotla/dotenv/pull/795)
- Add ability to find `[REDACTED_FILENAME].vault` file when filename(s) passed as an array [#784](https://github.com/motdotla/dotenv/pull/784)
...
- Optionally pass `DOTENV_KEY` to options rather than relying on `process[REDACTED_FILENAME].DOTENV_KEY`. Defaults to `process[REDACTED_FILENAME].DOTENV_KEY` [#754](https://github.com/motdotla/dotenv/pull/754)

## [16.2.0](https://github.com/motdotla/dotenv/compare/v16.1.4...v16.2.0) (2023-06-15)

### Added

- Optionally write to your own target object rather than `process[REDACTED_FILENAME]`. Defaults to `process[REDACTED_FILENAME]`. [#753](https://github.com/motdotla/dotenv/pull/753)
...
- Add `[REDACTED_FILENAME].vault` support. 🎉 ([#730](https://github.com/motdotla/dotenv/pull/730))

ℹ️ `[REDACTED_FILENAME].vault` extends the `[REDACTED_FILENAME]` file format standard with a localized encrypted vault file. Package it securely with your production code deploys. It's cloud agnostic so that you can deploy your secrets anywhere – without [risky third-party integrations](https://techcrunch.com/2023/01/05/circleci-breach/). [read more](https://github.com/motdotla/dotenv#-deploying)
...
- _Breaking:_ `#` marks the beginning of a comment (UNLESS the value is wrapped in quotes. Please update your `[REDACTED_FILENAME]` files to wrap in quotes any values containing `#`. For example: `SECRET_HASH="something-with-a-#-hash"`).

..Understandably, (as some teams have noted) this is tedious to do across the entire team. To make it less tedious, we recommend using [dotenv cli](https://github.com/dotenv-org/cli) going forward. It's an optional plugin that will keep your `[REDACTED_FILENAME]` files in sync between machines, environments, or team members.
...
- Log error on failure to load `[REDACTED_FILENAME]` file ([#594](https://github.com/motdotla/dotenv/pull/594))
...
- _Breaking_: default `path` is now `path.resolve(process.cwd(), '[REDACTED_FILENAME]')`
- _Breaking_: does not write over keys already in `process[REDACTED_FILENAME]` if the key has a falsy value
...
- Silent option to silence `console.log` when `[REDACTED_FILENAME]` missing

## [1.0.0] - 2015-03-13

### Removed

- support for multiple `[REDACTED_FILENAME]` files. should always use one `[REDACTED_FILENAME]` file for the current environment
