...
Dotenv es un módulo de dependencia cero que carga las variables de entorno desde un archivo `[REDACTED_FILENAME]` en [`process[REDACTED_FILENAME]`](https://nodejs.org/docs/latest/api/process.html#process_process_env). El almacenamiento de la configuración del entorno separado del código está basado en la metodología [The Twelve-Factor App](http://12factor.net/config).
...
Cree un archivo `[REDACTED_FILENAME]` en la raíz de su proyecto:
...
console.log(process[REDACTED_FILENAME]) // elimine esto después que haya confirmado que esta funcionando
```
...
Eso es todo. `process[REDACTED_FILENAME]` ahora tiene las claves y los valores que definiste en tu archivo `[REDACTED_FILENAME]`:
...
```bash
$ node -r dotenv/config tu_script.js dotenv_config_path=/custom/path/to/[REDACTED_FILENAME] dotenv_config_debug=true
```
...
```bash
$ DOTENV_CONFIG_ENCODING=latin1 DOTENV_CONFIG_DEBUG=true node -r dotenv/config tu_script.js dotenv_config_path=/custom/path/to/[REDACTED_FILENAME]
```
...
`Configuración` leerá su archivo `[REDACTED_FILENAME]`, analizará el contenido, lo asignará a [`process[REDACTED_FILENAME]`](https://nodejs.org/docs/latest/api/process.html#process_process_env),
...
Por defecto: `path.resolve(process.cwd(), '[REDACTED_FILENAME]')`
...
```js
require('dotenv').config({ path: '/personalizado/ruta/a/[REDACTED_FILENAME]' })
```
...
Anule cualquier variable de entorno que ya se haya configurada en su maquina con los valores de su archivo [REDACTED_FILENAME].
...
### ¿Por qué el archivo `[REDACTED_FILENAME]` no carga mis variables de entorno correctamente?

Lo más probable es que su archivo `[REDACTED_FILENAME]` no esté en el lugar correcto. [Vea este stack overflow](https://stackoverflow.com/questions/42335016/dotenv-file-is-not-loading-environment-variables).
...
### ¿Debo confirmar mi archivo `[REDACTED_FILENAME]`?

No. Recomendamos **enfáticamente** no enviar su archivo `[REDACTED_FILENAME]` a la versión de control. Solo debe incluir los valores especificos del entorno, como la base de datos, contraseñas o claves API.

### ¿Debería tener multiples archivos `[REDACTED_FILENAME]`?

No. Recomendamos **enfáticamente** no tener un archivo `[REDACTED_FILENAME]` "principal" y un archivo `[REDACTED_FILENAME]` de "entorno" como `[REDACTED_FILENAME].test`. Su configuración debe variar entre implementaciones y no debe compartir valores entre entornos.
...
Por defecto, nunca modificaremos ninguna variable de entorno que ya haya sido establecida. En particular, si hay una variable en su archivo `[REDACTED_FILENAME]` que colisiona con una que ya existe en su entorno, entonces esa variable se omitirá.

Si por el contrario, quieres anular `process[REDACTED_FILENAME]` utiliza la opción `override`.
...
Su código React se ejecuta en Webpack, donde el módulo `fs` o incluso el propio `process` global no son accesibles fuera-de-la-caja. El módulo `process[REDACTED_FILENAME]` sólo puede ser inyectado a través de la configuración de Webpack.
...
Sí! `dotenv.config()` devuelve un objeto que representa el archivo `[REDACTED_FILENAME]` analizado. Esto te da todo lo que necesitas para poder establecer valores en `process[REDACTED_FILENAME]`. Por ejemplo:
...
export default new Client(process[REDACTED_FILENAME].CLAVE_API)
...
`process[REDACTED_FILENAME].CLAVE_API` será vacio.
