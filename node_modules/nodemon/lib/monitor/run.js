...
function run(options) {
  ...
  const spawnOptions = {
    env: Object.assign({}, options.execOptions[REDACTED_FILENAME], process[REDACTED_FILENAME], {
      PATH:
        binPath +
        path.delimiter +
        (undefsafe(options, '.execOptions[REDACTED_FILENAME].PATH') || process[REDACTED_FILENAME].PATH),
    }),
    stdio: stdio,
  };
  ...
  if (utils.isWindows) {
    ...
    sh = process[REDACTED_FILENAME].comspec || 'cmd';
    shFlag = '/d /s /c';
  ...
  if (shouldFork) {
    ...
    var env = utils.merge(options.execOptions[REDACTED_FILENAME], process[REDACTED_FILENAME]);
    stdio.push('ipc');
    const forkOptions = {
      env: env,
      stdio: stdio,
